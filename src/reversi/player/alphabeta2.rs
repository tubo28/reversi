...
mod tests {
    ...
    // --- Head-to-head strength gate: AlphaBeta2 vs the baseline AlphaBeta. ---

    // Games per colour assignment (played for both colours -> 2 * GAMES total).
    const GAMES: u32 = 25;

    // Outcome of one game from AlphaBeta2's perspective.
    // 1 = AlphaBeta2 won, -1 = baseline won, 0 = draw.
    fn play(seed: u32, two_is_black: bool) -> i32 {
        // Distinct seeds so the two engines never share a random stream.
        let two = || Box::new(AlphaBeta2Player::new(seed));
        let base = || Box::new(AlphaBetaSearchPlayer::new(seed.wrapping_add(1_000_000)));
        let (mut gm, two_winner) = if two_is_black {
            (GameManager::new(two(), base()), Winner::Black)
        } else {
            (GameManager::new(base(), two()), Winner::White)
        };
        gm.verbose = false;
        gm.playout();
        let winner = &gm.result.as_ref().expect("game must be finished").winner;
        match winner {
            Winner::Draw => 0,
            _ if std::mem::discriminant(winner) == std::mem::discriminant(&two_winner) => 1,
            _ => -1,
        }
    }

    // Runs the seeded games in parallel. Returns (two_wins, base_wins, draws)
    // over 2 * GAMES games (each seed played with AlphaBeta2 as both colours).
    fn tally() -> (u32, u32, u32) {
        let threads = std::thread::available_parallelism()
            .map(|n| (n.get() - 1).max(1))
            .unwrap_or(1);
        let per_thread: Vec<(u32, u32, u32)> = std::thread::scope(|s| {
            let handles: Vec<_> = (0..threads)
                .map(|t| {
                    s.spawn(move || {
                        let (mut tw, mut bw, mut dr) = (0, 0, 0);
                        let mut i = t as u32;
                        let total = 2 * GAMES;
                        while i < total {
                            let seed = i / 2;
                            let two_is_black = i % 2 == 0;
                            match play(seed, two_is_black) {
                                1 => tw += 1,
                                -1 => bw += 1,
                                _ => dr += 1,
                            }
        ...
        per_thread.into_iter().fold((0, 0, 0), |(a, b, c), (x, y, z)| (a + x, b + y, c + z))
    }

    /// AlphaBeta2 must be at least as strong as the baseline: over these seeded
    /// head-to-head games it has to win strictly more than it loses (draws count
    /// for neither). Seeds are fixed, so this is deterministic.
    #[test]
    fn beats_baseline_alpha_beta() {
        let (two_wins, base_wins, draws) = tally();
        eprintln!(
            "head-to-head: AlphaBeta2 {two_wins} - {base_wins} baseline ({draws} draws) of {}",
            2 * GAMES
        );
        assert!(
            two_wins > base_wins,
            "AlphaBeta2 not stronger: won {two_wins}, lost {base_wins}, drew {draws} of {}",
            2 * GAMES
        );
    }
}
